...
lateinit var COURENT_USER: UserModel
...
inline fun initUser(crossinline function: () -> Unit) {
    if (AUTH.currentUser != null) {
        REF_DATABASE_ROOT.child(NODE_USERS).child(CURRENT_UID)
            .addListenerForSingleValueEvent(AppValueEventListener {
                COURENT_USER = it.getValue(UserModel::class.java) ?: UserModel()
                if (COURENT_USER.username.isEmpty()) {
                    REF_DATABASE_ROOT.child(NODE_USERS).child(CURRENT_UID).child(CHILD_USERNAME)
                        .setValue(
                            CURRENT_UID
                        ).addOnSuccessListener {
                            COURENT_USER.username = CURRENT_UID
                        }.addOnFailureListener { showToast(it.message.toString()) }
                }
                if (COURENT_USER.photoUrl.isEmpty()) {
                    REF_DATABASE_ROOT.child(NODE_USERS).child(CURRENT_UID).child(CHILD_USER_PHOTO)
                        .setValue(
                            "no_photo"
                        ).addOnSuccessListener {
                            COURENT_USER.photoUrl = "no_photo"
                        }.addOnFailureListener { showToast(it.message.toString()) }
                }
                function()
            })
...
inline fun initSearchedJobsMap(searchWord: String, crossinline function: () -> Unit) {
    ...
    JOBS_SEARCHED_MAP.apply {
        for ((k, v) in JOBS_HASHMAP) {
            ...
            for (j in v) {
                val st1 = j.name?.lowercase(Locale.getDefault())
                val st2 = searchWord.lowercase(Locale.getDefault())
...
fun deleteOldUsername(mNewUserName: String) {
    REF_DATABASE_ROOT.child(NODE_USERNAMES).child(COURENT_USER.username).removeValue()
        .addOnCompleteListener {
            if (it.isSuccessful) {
                showToast("Данные обновлены")
                APP_ACTIVITY.fragmentManager?.popBackStack()
                COURENT_USER.username = mNewUserName
            } else {
                showToast(it.exception?.message.toString())
            }
        ...
        REF_DATABASE_ROOT
            ...
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    ...
                    COURENT_USER.fullname = mFullName
                    APP_ACTIVITY.fragmentManager?.popBackStack()
                }
            }
            .addOnFailureListener { exception -> showToast(exception.message.toString()) }
